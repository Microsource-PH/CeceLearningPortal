...
import { HeroSection } from "@/components/LearningPortal/HeroSection"; // kept for potential fallback
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (["Creator", "Instructor"].includes(role)) {
      fetchCreatorData();
    }
  }, [user, userId]);
  ...
  const renderDashboard = () => {
    if (!isAuthenticated) {
      ...
      return (
        <>
          {/* Merged Hub hero at root for guests */}
          <HubHero
            showLearningCtas
            stats={{ resources: "500+", students: "2.1K", downloads: "15K" }}
          />
          <CourseSection />
          <StatsSection />
          <FeaturesSection />
    ...
    switch (user?.role) {
      case "Learner":
      case "Student" as any: // legacy fallback
      ...
      case "Creator":
      case "Instructor" as any: // legacy fallback
        ...
        const statsData = creatorStats || {
          ...
          totalStudents: coursesData.reduce(
            (sum, course) =>
              sum + (course.students || course.totalStudents || 0),
            0
          ),
          totalEarnings: coursesData.reduce((sum, course) => {
            const revenue =
              typeof course.revenue === "string"
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-2xl font-bold text-learning-warning">
                                {formatPHP(statsData.totalEarnings)}
                              </p>
                            </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-2xl font-bold text-primary">
                                {formatRating(statsData.avgRating)}
                              </p>
                            </div>
