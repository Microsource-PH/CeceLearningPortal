...
const Courses = () => {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        if (enrollmentsResponse.data) {
          setMyEnrollments(enrollmentsResponse.data as any[]);
        }
      }
    } catch (error) {
  ...
  const handleEnrollOrView = async (course: any) => {
    if (course.progress > 0) {
      ...
      // Use backend course data for accurate price and enrollment type
      const backendCourse = courses.find((c) => c.id === course.id);
      const priceNumber = backendCourse?.price ?? 0;
      const enrollmentType =
        backendCourse?.enrollmentType === "Subscription"
          ? "subscription"
          : "purchase";
  ...
  const renderContent = () => {
    ...
    if (user?.role === "Creator") {
      ...
      // Build proper CourseForDisplay shape for CreatorDashboard using backend data
      const instructorBackendCourses = courses.filter(
        (c) =>
          c.instructorId === user.id &&
          instructorCourses.some((ic) => ic.id === c.id)
      );

      const dashboardCourses = instructorBackendCourses.map((bc) => ({
        id: bc.id,
        title: bc.title,
        description: bc.description,
        students:
          allCourses.find((ac) => ac.id === bc.id)?.students ??
          bc.studentsCount,
        rating:
          allCourses.find((ac) => ac.id === bc.id)?.rating ?? (bc.rating || 0),
        revenue: formatPHP((bc.price || 0) * (bc.studentsCount || 0)),
        price: bc.price,
        originalPrice: bc.originalPrice,
        ...
        thumbnail:
          ...
          `https://images.unsplash.com/photo-1633356122544-f134324a6cee?w=400&h=250&fit=crop&auto=format`,
        status: bc.status,
        bestseller: bc.isBestseller,
        lastUpdated: "Recently",
        features:
          allCourses.find((ac) => ac.id === bc.id)?.features ||
          bc.features ||
          [],
        preview: undefined,
        enrollmentType:
          bc.enrollmentType === "Subscription" ? "subscription" : "one-time",
      }));

      const stats = {
        totalCourses: dashboardCourses.length,
        totalStudents: dashboardCourses.reduce(
          (sum, c) => sum + (c.students || 0),
          0
        ),
        totalEarnings: dashboardCourses.reduce(
          (sum, c) => sum + (c.price || 0) * (c.students || 0),
          0
        ),
        avgRating:
          dashboardCourses.reduce((sum, c) => sum + (c.rating || 0), 0) /
            (dashboardCourses.length || 1) || 0,
      };
                    ...
                    {course.originalPrice && (
                      <div className="text-sm text-muted-foreground line-through">
                        {typeof course.originalPrice === "string"
                          ? course.originalPrice
                          : formatPHP(course.originalPrice || 0)}
                      </div>
                    )}
                  </div>
                  {user?.role === "Admin" ? (
