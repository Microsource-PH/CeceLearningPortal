...
// New courses with proper GHL schema
const NEW_COURSES = [
  {
    // SPRINT COURSE
    ...
    enrollmentDeadline: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
    ...
    abandonmentSequenceEnabled: true,
  ...
  
  {
    ...
    endDate: new Date(Date.now() + 194 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
    weeklyLiveSession: true,
    ...
    completionCertificateEnabled: true,
    progressRemindersEnabled: true,
    abandonmentSequenceEnabled: false,
...
async function recreateSherwinCourses() {
  console.log('🔄 Recreating Sherwin\'s Courses with GHL Schema\n');
  ...
  try {
    ...
    if (sherwinCourses.length > 0) {
      ...
      for (const course of sherwinCourses) {
        try {
          ...
          console.log(`   ❌ Failed to delete: ${course.title} - ${error.message}`);
        }
      }
      console.log('');
    }
    ...
    if (successful > 0) {
      ...
      console.log('   - Sprint: Intensive daily learning with deadlines');
      console.log('   - Marathon: Cohort-based with payment plans and mentorship');
      console.log('   - Membership: Subscription model with ongoing content');
      console.log('   - Custom: Free course with flexible structure');
    }
