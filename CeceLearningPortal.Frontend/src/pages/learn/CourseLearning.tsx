...
const CourseLearning = () => {
  ...
  const [lessonProgress, setLessonProgress] = useState<Map<number, number>>(
    new Map()
  );
  ...
  const fetchCourseData = async () => {
    try {
      setLoading(true);
      console.log(
        "CourseLearning: Fetching course data for courseId:",
        courseId
      );
      ...
      if (response.data) {
        ...
        if ((response as any).data.completedLessons) {
          const comp = (response as any).data.completedLessons as number[];
          setCompletedLessons(new Set(comp));
        }
        ...
        calculateProgress(data, savedCompletedLessons, savedLessonProgress);

        // Update course status in localStorage
        const hasStarted =
          savedCompletedLessons.size > 0 ||
          Array.from(savedLessonProgress.values()).some((p) => p > 0);
        if (!hasStarted) {
          // First time accessing the course
          updateCourseStatusInStorage(
            parseInt(courseId!),
            "in_progress",
            0,
            totalLessons
  ...
  const updateCourseStatusInStorage = (
    courseId: number,
    status: string,
    progress?: number,
    totalLessons?: number
    ...
    if (courseIndex >= 0) {
      ...
      if (progress !== undefined) {
        ...
        enrolledCourses[courseIndex].completedLessons = Math.round(
          (progress / 100) * total
        );
      }
  ...
  const calculateProgress = (
    ...
    lessonProgressMap?: Map<number, number>
  ) => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="flex h-[calc(100vh-64px)]">
        {/* Sidebar */}
        <div
          className={`${sidebarOpen ? "w-80" : "w-0"} transition-all duration-300 border-r overflow-hidden`}
        >
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p
                                className={`text-sm truncate ${isActive ? "" : isCompleted ? "text-muted-foreground" : ""}`}
                              >
                                {lessonIndex + 1}. {lesson.title}
                              </p>
                              <div className="flex items-center gap-2">
                                <p
                                  className={`text-xs ${isActive ? "opacity-80" : "text-muted-foreground"}`}
                                >
                                  {lesson.duration}
                                </p>
                  ...
                  <div className="aspect-video bg-black rounded-lg overflow-hidden">
                    ...
                    currentLesson.videoUrl.includes("youtu.be") ? (
                      ...
                      <video
                        ...
                        className="w-full h-full"
                        onTimeUpdate={(e) =>
                          handleVideoTimeUpdate(e, currentLesson.id)
                        }
                        onLoadedMetadata={(e) =>
                          handleVideoLoaded(e, currentLesson.id)
                        }
                        onEnded={() =>
                          !isLessonCompleted &&
