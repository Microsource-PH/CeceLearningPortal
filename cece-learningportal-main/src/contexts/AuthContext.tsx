...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (email === 'instructor@example.com' || email === 'dr.johnson@example.com') {
        ...
        localStorage.setItem('drJohnsonCourses', JSON.stringify(drJohnsonCourses));
      }
      ...
      if (email === 'sarah.wilson@example.com') {
        ...
        const enrolledCourses = [
          ...
          },
          {
            ...
            enrollmentType: "subscription",
            price: 0,
            certificateUrl: "https://example.com/certificates/sarah-cv-fundamentals"
          },
          {
            id: 3,
            courseId: 57,
  ...
  const register = async (email: string, password: string, fullName: string, role: UserRole = 'Learner'): Promise<{ success: boolean; error?: string }> => {
    try {
      // Map UserRole to registration role (Learner -> Student, Instructor -> Creator)
      const registrationRole = role === 'Instructor' ? 'Creator' : 'Student';
      const response = await api.register(email, password, fullName, registrationRole);
      ...
      if (authData.status === 'PendingApproval') {
        return { 
          success: true, 
          error: 'Your registration is pending admin approval. You will receive an email once your account is approved.' 
        };
      }
