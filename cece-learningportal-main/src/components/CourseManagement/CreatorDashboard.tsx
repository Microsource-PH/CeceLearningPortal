...
export const CreatorDashboard = ({ 
  ...
  const [editingCourse, setEditingCourse] = useState<CourseForDisplay | undefined>();
  ...
  const handleEditCourse = (course: CourseForDisplay) => {
    ...
    setShowEditor(true);
    console.log('showEditor set to true, editingCourse:', course);
  };

  const handleViewCourse = async (course: CourseForDisplay) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        // Fallback to using existing course data
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <div className="bg-black/70 rounded-lg p-2">
                      <div className="flex justify-between text-white text-xs mb-1">
                        <span>Status</span>
                        <span className={`${
                          course.status === 'Active' || course.status === 'active' ? 'text-green-400' : 
                          course.status === 'Draft' || course.status === 'draft' ? 'text-yellow-400' : 
                          course.status === 'PendingApproval' ? 'text-blue-400' : 'text-gray-400'
                        }`}>
                          {course.status === 'Active' || course.status === 'active' ? 'Published' : 
                           course.status === 'Draft' || course.status === 'draft' ? 'Draft' : 
                           course.status === 'PendingApproval' ? 'Pending Approval' : 'Unpublished'}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <CardContent className="pt-0 space-y-4">
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>{formatPHP(course.revenue)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <div className="flex gap-1">
                      <Button 
                        ...
                        size="sm"
                        onClick={() => {
                          alert('Edit clicked for course: ' + course.title);
                      ...
                      {(course.status === 'Draft' || course.status === 'draft') && onPublishCourse && (
                        <Button 
                          variant="outline" 
                          size="sm"
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-learning-blue">{stats.totalCourses}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-learning-success">{stats.totalStudents.toLocaleString()}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-learning-warning">{formatPHP(stats.totalEarnings)}</p>
                  </div>
