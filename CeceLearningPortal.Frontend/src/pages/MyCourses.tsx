...
const MyCourses = () => {
  ...
  const fetchEnrolledCourses = async () => {
    try {
      ...
      if (result.data) {
        ...
        const transformedCourses: EnrolledCourse[] = storedCourses.map(
          (course: any) => ({
            ...
            thumbnail:
              course.thumbnail ||
              `https://images.unsplash.com/photo-${Math.floor(Math.random() * 1000000000)}?w=400&h=250&fit=crop&auto=format`,
  ...
  const getCourseStats = () => {
    ...
    const avgProgress =
      total > 0
        ? Math.round(
            enrolledCourses.reduce((sum, c) => sum + c.progress, 0) / total
          )
        : 0;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "completed":
        return (
          <Badge className="bg-learning-success text-white">Completed</Badge>
        );
      case "in_progress":
        return (
          <Badge className="bg-learning-warning text-white">In Progress</Badge>
        );
      case "not_started":
  ...
  const getNextLessonButton = (course: EnrolledCourse) => {
    ...
    if (course.nextLesson) {
      return (
        <Button
          size="sm"
          onClick={() => navigate(`/learn/course/${course.courseId}`)}
        >
                ...
                {filteredCourses.map((course) => (
                  ...
                  >
                    ...
                    <CardContent className="pt-0 space-y-4">
                      ...
                      <div className="pt-2">{getNextLessonButton(course)}</div>
                    </CardContent>
                  </Card>
                ))}
              </div>
