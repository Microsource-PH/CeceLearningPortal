...
const Marketplace = () => {
  ...
  const [editingCourse, setEditingCourse] = useState<CourseForDisplay | null>(null);
  ...
  useEffect(() => {
    if (editCourseId && (courses.length > 0 || unfilteredCourses.length > 0)) {
      ...
      if (!courseToEdit && (user?.role === 'Creator' || user?.role === 'Instructor')) {
  ...
  useEffect(() => {
    const loadInstructors = async () => {
      ...
      try {
        ...
        if (analyticsResult.data?.topPerformers && analyticsResult.data.topPerformers.length > 0) {
          ...
          const instructorsData = await Promise.all(
            analyticsResult.data.topPerformers.map(async (performer: any) => {
              ...
              try {
                ...
                const coursesResult = await courseService.getCourses({ instructorId: performer.instructorId });
                
                if (coursesResult.data && coursesResult.data.length > 0) {
                  instructorCourses = coursesResult.data;
                  console.log(`Fetched ${instructorCourses.length} courses for ${performer.instructorName}`);
                } else {
                  // Fallback to local courses
                  instructorCourses = courses.filter(c => 
                    c.instructorId === performer.instructorId || 
                    c.instructor_id === performer.instructorId ||
                    c.instructorName === performer.instructorName
                  );
                  console.log(`Using local courses for ${performer.instructorName}: ${instructorCourses.length} courses`);
                }
              } catch (error) {
                console.log('Failed to fetch instructor courses, using local data:', error);
                instructorCourses = courses.filter(c => 
                  c.instructorId === performer.instructorId || 
                  c.instructor_id === performer.instructorId ||
                  c.instructorName === performer.instructorName
              ...
              return {
                ...
                courses: instructorCourses.slice(0, 4).map(course => ({
                  ...
                  students: course.studentsCount || course.students || course.total_students || 0,
                  rating: course.averageRating || course.rating || course.average_rating || 0,
          ...
          console.log('Setting instructors from analytics with real data:', instructorsData);
          ...
          courses.forEach(course => {
            ...
            const instructorId = course.instructorId || course.instructor_id || instructorName;
          ...
          const instructorsData = Array.from(instructorMap.values()).map(instructor => {
            const instructorCourses = instructor.courses;
            
            // Calculate stats from courses
            const totalStudents = instructorCourses.reduce((sum, c) => sum + (c.studentsCount || c.students || 0), 0);
            const totalRevenue = instructorCourses.reduce((sum, c) => sum + ((c.studentsCount || c.students || 0) * (c.price || 0)), 0);
            const avgRating = instructorCourses.length > 0 
              ? instructorCourses.reduce((sum, c) => sum + (c.averageRating || c.rating || 0), 0) / instructorCourses.length 
              : 0;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (coursesResult.data) {
        ...
        const coursesWithInstructorIds = coursesResult.data.map(course => {
          ...
          if (!course.instructorId) {
            ...
            if (instructorName) {
              const instructor = allUsers.find(u => 
                u.fullName === instructorName || 
                u.name === instructorName ||
                (u.email === 'instructor@example.com' && instructorName === 'Dr. Sarah Johnson')
              );
              if (instructor) {
        ...
        setUnfilteredCourses(coursesWithInstructorIds);
        
        // Debug: Check all statuses
        console.log('All course statuses:', coursesWithInstructorIds.map(c => ({ 
          title: c.title, 
          status: c.status,
        ...
        const activeCourses = coursesWithInstructorIds.filter(course => {
          const isActive = course.status === 'Active' || 
                          course.status === 'active' || 
                          course.status === 'Published' ||
                          course.status === 'published';
          if (!isActive && course.isPublished) {
            console.log('Published course with non-active status:', course.title, 'Status:', course.status);
          }
          return isActive;
        });
        
        setCourses(activeCourses);
        console.log('Active courses found:', activeCourses.length, 'out of', coursesWithInstructorIds.length);
        console.log('Courses with instructor IDs:', coursesWithInstructorIds);
        console.log('Sample course:', coursesWithInstructorIds[0]);
        console.log('Course types:', coursesWithInstructorIds.map(c => ({ title: c.title, courseType: c.courseType })));
      }
      ...
      if (user?.role === 'Admin') {
        ...
        if (analyticsResult.data) {
          ...
          setPlatformStats({
            ...
            activeSubscriptions: analyticsResult.data.summary.activeSubscriptions,
      ...
      if (user && userId) {
        if (user.role === 'Student' || user.role === 'Learner') {
          ...
          if (coursesResult.data) {
            ...
            console.error('Failed to fetch enrolled courses:', coursesResult.error);
          ...
          const mergedCourses = enrolledData.map((course: any) => {
            const stored = storedCourses.find((s: any) => s.courseId === course.courseId);
            if (stored) {
          ...
          const subscriptionResult = await subscriptionService.getMySubscription();
        ...
        } else if (user.role === 'Creator' || user.role === 'Instructor') {
          ...
          if (analyticsResult.data) {
            setCreatorStats({
              ...
              totalEarnings: revenueResult.data?.creatorEarnings || analyticsResult.data.summary.totalRevenue * 0.8,
              totalRevenue: analyticsResult.data.summary.totalRevenue,
      ...
      courses: [
        {
          ...
          description: "Complete React course from beginner to advanced with real projects",
          students: 1892,
          rating: 4.90,
          revenue: "₱472,500",
          price: 1899.00,
          ...
          features: ["Lifetime Access", "Certificate", "Mobile Access", "Source Code"],
  ...
  const allCourses = courses.length > 0 ? courses.map(course => ({
    ...course,
    ...
    thumbnail: course.thumbnail || `https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=400&h=250&fit=crop&auto=format`,
    features: course.features || ['Online Access', 'Certificate of Completion']
  ...
  const getTopPerformers = () => {
    if (creators.length > 0) {
      ...
      const creatorsWithStats = creators.map(creator => {
        const creatorCourses = courses.filter(c => c.instructor_id === creator.id);
        const totalRevenue = creatorCourses.reduce((sum, course) => sum + (course.totalRevenue || 0), 0);
        const totalStudents = creatorCourses.reduce((sum, course) => sum + (course.totalStudents || 0), 0);
        const avgRating = creatorCourses.length > 0 
          ? creatorCourses.reduce((sum, course) => sum + (course.rating || 0), 0) / creatorCourses.length
          : 0;
  ...
  const calculateRealStats = () => {
    ...
    const uniqueInstructors = new Set(courses.map(c => c.instructorId || c.instructor_id)).size;
    ...
    return {
      totalContributors: uniqueInstructors || creators.length || contributors.length,
  ...
  const handlePurchaseCourse = async (courseId: number, price: number, courseName: string) => {
    if (!user) {
      navigate('/login');
      return;
    }
    ...
    try {
      ...
      const result = await enrollInCourse(courseId, courseName, price, 'purchase');
  ...
  const handleSubscribe = async (planId: string, planName: string, price: number) => {
    ...
    try {
      ...
      const subscriptionData = {
        ...
        endDate: new Date(Date.now() + (planId === 'annual' ? 365 : 30) * 24 * 60 * 60 * 1000).toISOString(),
      ...
      const sampleCourses = [
        { id: 55, name: "Complete Web Development Bootcamp 2025", price: 89.99 },
        { id: 56, name: "Advanced React & Redux Masterclass", price: 79.99 },
        { id: 57, name: "Machine Learning with Python & TensorFlow", price: 99.99 }
      ];

      for (const course of sampleCourses) {
        await enrollInCourse(course.id, course.name, course.price, 'subscription');
      }
  ...
  const creatorStatsDisplay = creatorStats || {
    ...
    totalEarnings: parseInt(contributors[0]?.creatorEarnings?.replace(/[₱,]/g, '') || '0'),
    avgRating: 4.8
  };
    ...
    try {
      ...
      if (response.data) {
        toast({
          ...
          description: "Your course has been submitted for admin approval. You will be notified once it's approved.",
        });
  ...
  const seedSarahJohnsonData = async () => {
    ...
    try {
      ...
      if (!sarah) {
        ...
        sarah = newSarah;
        console.log('Created Sarah Johnson account with email: sarah.johnson@example.com');
        ...
        const existingUsers = JSON.parse(localStorage.getItem('users') || '[]');
        const userIndex = existingUsers.findIndex((u: any) => u.id === sarah.id);
        if (userIndex >= 0) {
      ...
      // Get all students
      const students = users.data?.filter(u => u.role === 'Student' || u.role === 'Learner').slice(0, 10) || [];
      
      // Get Sarah's courses
      const allCourses = await DatabaseService.getCourses();
      const sarahCourses = allCourses.data?.filter(c => 
        c.instructorName === 'Dr. Sarah Johnson' || 
        c.instructor === 'Dr. Sarah Johnson'
      ) || [];
      
      if (sarahCourses.length === 0) {
        console.log('No courses found for Sarah. The initial seed should have created them.');
        return;
      }
      ...
      const filteredEnrollments = existingEnrollments.filter((e: any) => 
        !sarahCourses.some(c => c.id === e.courseId)
      );
      const filteredPayments = existingPayments.filter((p: any) => 
        !sarahCourses.some(c => c.id === p.courseId)
      );
      const filteredReviews = existingReviews.filter((r: any) => 
        !sarahCourses.some(c => c.id === r.courseId)
      );
      ...
      for (const course of sarahCourses) {
        ...
        for (let i = 0; i < enrollmentCount && i < students.length * 5; i++) {
          ...
          if (i < enrollmentCount * 0.4) {
            ...
            completedAt = new Date(new Date(enrollmentDate).getTime() + 30 * 24 * 60 * 60 * 1000).toISOString();
          ...
          if (progressPercentage === 100 && Math.random() > 0.3) {
            const review = {
              ...
              rating: isAdvancedCourse ? 4.5 + Math.random() * 0.5 : 4.3 + Math.random() * 0.7,
      ...
      console.log(`Total enrollments created: ${filteredEnrollments.length - existingEnrollments.length}`);
      console.log(`Total payments created: ${filteredPayments.length - existingPayments.length}`);
      console.log(`Total reviews created: ${filteredReviews.length - existingReviews.length}`);
  ...
  const renderContent = () => {
    if (user?.role === 'Creator' || user?.role === 'Instructor') { // Support both new and old role names
              ...
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {(() => {
                    ...
                    const userCourses = unfilteredCourses.filter(c => 
                    ...
                    <Card key={course.id} className="group hover:shadow-card-hover transition-all duration-300 overflow-hidden flex flex-col h-full">
                      ...
                      <div className="relative overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4 flex flex-col gap-2 max-w-[calc(100%-2rem)]">
                          ...
                          {!!course.courseType && (
                            <Badge className={`${
                              ...
                              {course.courseType === 'Sprint' ? <Zap className="w-3 h-3 mr-1" /> : null}
                              {course.courseType === 'Marathon' ? <TrendingUp className="w-3 h-3 mr-1" /> : null}
                              {course.courseType === 'Membership' ? <Crown className="w-3 h-3 mr-1" /> : null}
                              {course.courseType === 'Custom' ? <Star className="w-3 h-3 mr-1" /> : null}
                              {course.courseType}
                            </Badge>
                          )}
                        ...
                        <div className="absolute bottom-4 right-4">
                          <Badge className={`${
                            course.status === 'Active' || course.status === 'active' ? 'bg-green-500/90 text-white' : 
                            course.status === 'Draft' || course.status === 'draft' ? 'bg-yellow-500/90 text-white' : 
                            course.status === 'PendingApproval' || course.status === 'pendingapproval' ? 'bg-blue-500/90 text-white' : 
                            'bg-gray-500/90 text-white'
                          } shadow-lg border-0 text-xs`}>
                            {course.status === 'Active' || course.status === 'active' ? 'Published' : 
                             course.status === 'Draft' || course.status === 'draft' ? 'Draft' : 
                             course.status === 'PendingApproval' || course.status === 'pendingapproval' ? 'Pending Approval' : 'Unpublished'}
                          </Badge>
                        </div>
                      </div>
                      ...
                      <CardContent className="pt-0 space-y-4 flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex items-center justify-between py-2 px-3 bg-muted/50 rounded-lg">
                          ...
                          <span className="text-sm font-semibold">{formatPHP((course.studentsCount || course.students || 0) * (course.price || 0))}</span>
                        </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-learning-blue">{creatorStatsDisplay.totalCourses}</p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-learning-success">{creatorStatsDisplay.totalStudents.toLocaleString()}</p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-learning-warning">{formatPHP(creatorStatsDisplay.totalEarnings)}</p>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-primary">{formatRating(creatorStatsDisplay.avgRating)}</p>
                      </div>
          ...
          <div className="mb-8">
              ...
              <Card className="bg-gradient-card shadow-card">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-muted-foreground">Total Courses</p>
                      <p className="text-2xl font-bold text-primary">{studentStatsDisplay.totalCourses}</p>
                      <p className="text-xs text-muted-foreground mt-1">{studentStatsDisplay.inProgressCourses || 0} in progress</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-learning-success">{studentStatsDisplay.completedCourses}</p>
                      <p className="text-xs text-muted-foreground mt-1">{studentStatsDisplay.averageScore}% avg score</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-learning-warning">{studentStatsDisplay.learningHours}</p>
                      <p className="text-xs text-muted-foreground mt-1">{studentStatsDisplay.currentStreak} day streak</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-learning-blue">{formatPHP(studentStatsDisplay.totalSpent)}</p>
                      <p className="text-xs text-muted-foreground mt-1">{studentStatsDisplay.totalCertificates} certificates</p>
                    </div>
            ...
            {activeSubscription ? (
                  ...
                  <div className="flex items-center justify-between p-4 bg-gradient-card rounded-lg">
                    <div>
                      <h3 className="font-semibold text-lg">
                        {activeSubscription.planId === 'premium' ? 'Premium Plan' : 
                         activeSubscription.planId === 'basic' ? 'Basic Plan' : 
                         activeSubscription.planId === 'annual' ? 'Annual Premium' : 'Subscription'}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        {activeSubscription.planId === 'basic' ? 'Access to 100+ courses' : 'Unlimited access to all courses'}
                      </p>
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-bold text-primary">
                        ...
                        {activeSubscription.planId === 'annual' ? '/year' : '/month'}
                      </p>
                      <Badge className="bg-learning-success text-white">Active</Badge>
                    </div>
                  </div>
              ...
              <Card className="mb-8 shadow-card">
                ...
                <CardContent>
                  <p className="text-muted-foreground mb-4">Get unlimited access to all courses with a subscription plan</p>
                  <Button className="w-full" onClick={() => {
                    const element = document.getElementById('subscriptions-tab');
                ...
                <CardContent>
                  <div className="space-y-4">
                    {enrolledCourses.slice(0, 3).map((course) => (
                      <div key={course.id} className="flex items-center gap-4 p-4 bg-gradient-card rounded-lg hover:shadow-md transition-shadow">
                        <img 
    ...
    <>
      ...
      <div className="mb-8">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-primary">{totalStats.totalContributors}</p>
                  </div>
            ...
            <Card className="bg-gradient-card shadow-card">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground">Total Courses</p>
                    <p className="text-2xl font-bold text-learning-blue">{totalStats.totalCourses}</p>
                  </div>
                  <BookOpen className="w-8 h-8 text-learning-blue/60" />
                </div>
              </CardContent>
            ...
            <Card className="bg-gradient-card shadow-card">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-muted-foreground">Total Students</p>
                    <p className="text-2xl font-bold text-learning-success">{totalStats.totalStudents.toLocaleString()}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-learning-warning">{formatPHP(totalStats.totalRevenue)}</p>
                  </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {topPerformers.map((contributor, index) => (
                  <div key={contributor.id} className="flex items-center space-x-4 p-4 bg-gradient-card rounded-lg">
                    <div className="relative">
                      <Avatar className="w-12 h-12">
                        <AvatarImage src={contributor.avatar || contributor.avatar_url} alt={contributor.name || contributor.full_name} />
                        <AvatarFallback>{(contributor.name || contributor.full_name).split(' ').map(n => n[0]).join('')}</AvatarFallback>
                      </Avatar>
                      {index === 0 && (
                    ...
                    <div className="flex-1">
                      <p className="font-semibold">{contributor.name || contributor.full_name}</p>
                      <p className="text-sm text-muted-foreground">{contributor.title || 'Creator'}</p>
                      <div className="flex items-center gap-1 mt-1">
                        <Star className="w-4 h-4 text-learning-warning fill-current" />
                        <span className="text-sm font-medium">{formatRating(contributor.rating || 0)}</span>
                        <span className="text-sm text-muted-foreground">
                          • {(contributor.totalStudents || 0).toLocaleString()} students
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <Tabs defaultValue="courses" className="w-full">
          <TabsList className={`grid w-full ${!isCreator && !isStudent ? 'grid-cols-4' : 'grid-cols-3'}`}>
            <TabsTrigger value="courses">Browse Courses</TabsTrigger>
            <TabsTrigger value="subscriptions" id="subscriptions-tab">Subscription Plans</TabsTrigger>
                ...
                <Card key={course.id} className="group hover:shadow-card-hover transition-all duration-300 overflow-hidden flex flex-col h-full">
                  ...
                  <CardHeader className="pb-4">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <Avatar className="w-6 h-6 flex-shrink-0">
                        <AvatarImage src={course.instructorAvatar} alt={course.instructor || 'Instructor'} />
                        <AvatarFallback className="text-xs">{(course.instructor || 'UI').split(' ').map(n => n[0]).join('')}</AvatarFallback>
                      </Avatar>
                      <span className="text-sm text-muted-foreground truncate">by {course.instructor || 'Unknown Instructor'}</span>
                      {!!course.verified && (
                  ...
                  <CardContent className="pt-0 space-y-4 flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between text-sm text-muted-foreground">
                      <div className="flex items-center gap-1 min-w-0">
                        <Star className="w-4 h-4 fill-yellow-400 text-yellow-400 flex-shrink-0" />
                        <span className="font-medium">{formatRating(course.rating || 0)}</span>
                        {!!course.students && course.students > 0 && (
                          <span className="truncate">({course.students.toLocaleString()})</span>
                        )}
                      </div>
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <Clock className="w-4 h-4" />
                    ...
                    <div className="flex items-center justify-between pt-2 gap-2">
                      <div className="flex flex-col">
                        <span className="text-lg font-bold text-learning-blue">{formatPHP(course.price || 0)}</span>
                        {!!course.originalPrice && course.originalPrice !== course.price && (
                          <span className="text-xs text-muted-foreground line-through">
                            {formatPHP(course.originalPrice)}
                          </span>
                      ...
                      <Button 
                        onClick={() => handlePurchaseCourse(course.id, course.price || 0, course.title || 'Course')}
                        size="sm"
          ...
          <TabsContent value="subscriptions" className="mt-8">
            <div className="max-w-4xl mx-auto">
              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold mb-2">Choose Your Learning Plan</h2>
                <p className="text-muted-foreground">Get unlimited access to our entire course library</p>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {subscriptionPlans.map((plan) => (
                  <Card key={plan.id} className={`relative shadow-card hover:shadow-card-hover transition-all duration-300 ${plan.popular ? 'ring-2 ring-primary' : ''}`}>
                    {!!plan.popular && (
          ...
          <TabsContent value="instructors" className="mt-8">
            ...
            {loading ? (
              ...
              <Card className="p-8 text-center">
                <p className="text-muted-foreground">No instructors found with published courses.</p>
              </Card>
            ) : (
              <div className="space-y-6">
                {instructors.map((contributor) => (
                <Card key={contributor.id} className="shadow-card hover:shadow-card-hover transition-all duration-300">
                  <CardContent className="p-6">
                    <div className="flex flex-col lg:flex-row gap-6">
                      ...
                      <div className="lg:w-1/3">
                        <div className="flex items-start gap-4 mb-4">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-muted-foreground mb-2">{contributor.title}</p>
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                Joined {new Date(contributor.joinDate).getFullYear()}
                              </div>
                            </div>
                          </div>
                        </div>

                        {/* Revenue Breakdown - Only show for Admin and Instructors */}
                        {(user?.role === 'Admin' || user?.role === 'Instructor' || user?.role === 'Creator') && (
                          <div className="grid grid-cols-2 gap-4 mb-4">
                            <div className="text-center p-3 bg-gradient-card rounded-lg">
                              <p className="text-xl font-bold text-learning-success">{formatPHP(contributor.creatorEarnings)}</p>
                              <p className="text-xs text-muted-foreground">Creator Earnings (80%)</p>
                            </div>
                            <div className="text-center p-3 bg-gradient-card rounded-lg">
                              <p className="text-xl font-bold text-learning-warning">{formatPHP(contributor.platformShare)}</p>
                              <p className="text-xs text-muted-foreground">Platform Share (20%)</p>
                            </div>
                          </div>
                        )}

                        {/* Overall Stats - Always visible but different layout for students */}
                        <div className={`grid ${(user?.role === 'Student' || user?.role === 'Learner') ? 'grid-cols-3' : 'grid-cols-2'} gap-4`}>
                          <div className="text-center p-3 bg-gradient-card rounded-lg">
                            <p className="text-2xl font-bold text-primary">{contributor.totalCourses}</p>
                            <p className="text-sm text-muted-foreground">Courses</p>
                          </div>
                          <div className="text-center p-3 bg-gradient-card rounded-lg">
                            <div className="flex items-center justify-center gap-1 mb-1">
                              <Star className="w-4 h-4 text-learning-warning fill-current" />
                              <span className="text-xl font-bold">{formatRating(contributor.rating || 0)}</span>
                            </div>
                          ...
                          {(user?.role === 'Student' || user?.role === 'Learner') && (
                            <div className="text-center p-3 bg-gradient-card rounded-lg">
                              <p className="text-2xl font-bold text-learning-blue">{contributor.totalStudents.toLocaleString()}</p>
                      ...
                      <div className="lg:w-2/3">
                        <h4 className="text-lg font-semibold mb-4">Published Courses</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {contributor.courses.map((course, index) => (
                            <div key={index} className="p-4 border border-border rounded-lg bg-gradient-card">
                              <div className="flex justify-between items-start mb-3">
                                ...
                                <div className="text-right">
                                  <p className="font-semibold text-primary">{formatPHP(course.price)}</p>
                                  <Badge variant={course.status === 'Active' || course.status === 'active' ? 'default' : 'secondary'} className="mt-1">
                                    {course.status === 'Active' || course.status === 'active' ? 'Published' : 
                                     course.status === 'Draft' || course.status === 'draft' ? 'Draft' : 
                                     course.status === 'PendingApproval' || course.status === 'pendingapproval' ? 'Pending' : course.status}
                                  </Badge>
                                </div>
                              </div>
                              <Button 
                                onClick={() => handlePurchaseCourse(course.id, course.price, course.title)}
                                size="sm" 
                                className="w-full"
                              >
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-primary">{formatPHP(totalStats.platformRevenue)}</p>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-learning-success">{formatPHP(totalStats.creatorEarnings)}</p>
                            <p className="text-xs text-muted-foreground mt-1">80% Share</p>
                          </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-learning-blue">
                              {formatPHP(courses.length > 0 ? courses.reduce((sum, c) => sum + (c.price || 0), 0) / courses.length : 0)}
                            </p>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-learning-warning">{totalStats.totalStudents}</p>
                    ...
                    <CardContent className="p-6">
                      <div className="space-y-4">
                        {courses.slice(0, 5).map((course) => {
                          const revenue = (course.studentsCount || 0) * (course.price || 0);
                          const revenuePercentage = totalStats.totalRevenue > 0 ? (revenue / totalStats.totalRevenue) * 100 : 0;
                          
                          return (
                            <div key={course.id} className="p-4 bg-gradient-to-r from-background to-muted/20 rounded-lg border border-border">
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <div className="text-right">
                                  <p className="font-semibold">{formatPHP(revenue)}</p>
                                  <p className="text-xs text-muted-foreground">{course.studentsCount || 0} students</p>
                                </div>
                              </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {(() => {
                      ...
                      if (platformStats?.popularCategories && platformStats.popularCategories.length > 0) {
                        return platformStats.popularCategories.map((cat) => (
                          <Card key={cat.category} className="bg-gradient-card shadow-card hover:shadow-card-hover transition-all duration-300">
                            <CardContent className="p-6">
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-medium">{cat.enrollmentCount}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  <span className="text-muted-foreground">Revenue</span>
                                  <span className="font-medium text-learning-success">{formatPHP(cat.revenue)}</span>
                                </div>
                              </div>
                            </CardContent>
                          </Card>
                      ...
                      const categoryStats = courses.reduce((acc, course) => {
                        ...
                        acc[category].count++;
                        acc[category].revenue += (course.studentsCount || 0) * (course.price || 0);
                        acc[category].students += course.studentsCount || 0;
                        return acc;
                      }, {} as Record<string, { count: number; revenue: number; students: number }>);
                      
                      return Object.entries(categoryStats).map(([category, stats]) => (
                        <Card key={category} className="bg-gradient-card shadow-card hover:shadow-card-hover transition-all duration-300">
                          <CardContent className="p-6">
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex justify-between text-sm">
                                <span className="text-muted-foreground">Revenue</span>
                                <span className="font-medium text-learning-success">{formatPHP(stats.revenue)}</span>
                              </div>
                            </div>
                          </CardContent>
                        </Card>
                ...
                {platformStats?.subscriptionMetrics && (
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-2xl font-bold text-primary">{platformStats.subscriptionMetrics.totalActive}</p>
                            </div>
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-2xl font-bold text-learning-success">{formatPHP(platformStats.subscriptionMetrics.monthlyRecurringRevenue)}</p>
                            </div>
                      ...
                      {platformStats.subscriptionMetrics.byPlan && platformStats.subscriptionMetrics.byPlan.length > 0 && (
                        <Card className="bg-gradient-card shadow-card">
                          <CardContent className="p-6">
                            <h4 className="font-semibold mb-3">By Plan</h4>
                            <div className="space-y-2">
                              {platformStats.subscriptionMetrics.byPlan.map((plan) => (
                                <div key={plan.plan} className="flex justify-between text-sm">
                                  <span className="text-muted-foreground">{plan.plan}</span>
                                  <span className="font-medium">{plan.count} ({formatPHP(plan.monthlyRevenue)})</span>
                                </div>
                              ))}
                            </div>
                          </CardContent>
  ...
  if (showCourseEditor && editingCourse) {
    console.log('Rendering CourseEditorWithFeatures for course:', editingCourse);
    return (
      <div className="min-h-screen bg-background">
        <Navigation />
        <div className="container mx-auto px-6 py-8">
            ...
            <Button 
              ...
              onClick={async () => {
                try {
                  toast({
                    title: "Seeding Data",
                    description: "Creating realistic data for Dr. Sarah Johnson...",
                  });
                  ...
                  try {
                    ...
                    if (registerResponse.data) {
                      console.log('Sarah Johnson registered in backend:', registerResponse.data);
                      
                      // For demo purposes, we'll use the existing instructor account
                      toast({
                        title: "Account Created",
                        description: "Sarah Johnson has been registered. Please use instructor@example.com / instructor123 to login.",
                      });
                    }
                  } catch (error) {
                    console.log('Sarah may already exist or backend registration failed, continuing with local seed...');
                  }
                  
                  await seedSarahJohnsonData();
                  toast({
                    title: "Data Seeded Successfully",
                    description: "Sarah Johnson's courses, enrollments, and payments have been created.",
                  });
                  // Refresh data
                  await fetchData();
                } catch (error) {
                  toast({
                    ...
                    description: "Could not seed data. Please check the console.",
                    variant: "destructive"
                  });
